...
// Query: *[    _type == "marketSegment" &&    defined(slug.current) &&    count(      *[        _type == "productCategory" &&        marketSegment._ref == ^._id &&         count(*[_type == "organization" && references(^._id)]) > 0      ]    ) > 0  ] | order(lower(name) asc) [0...20] {      _id,  name,  "slug": slug.current,  description,    "productCategories": *[_type == "productCategory" && marketSegment._ref == ^._id && count(*[_type == "organization" && references(^._id)]) > 0] | order(lower(name) asc) {      _id,      name,      "slug": slug.current,      expansion,    }  }
...
// Query: *[    _type == "siteSettings" &&    _id == "siteSettings"  ] [0] {    title,    description,    copyright,    navigation[] {      name,      href,    },  }
