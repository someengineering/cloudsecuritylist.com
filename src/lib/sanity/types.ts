...
// Variable: PAGE_QUERY
// Query: *[_type == "page" && slug.current == $slug][0] {    _createdAt,    "_updatedAt":   [    _updatedAt,    select(defined(listType) => *[_type == ^.listType] | order(_updatedAt desc) [0]._updatedAt)  ] [defined(@)] | order(@ desc) [0],      "slug": slug.current,  title,  description,  "icon": icon.name,  ...select(!defined(listType) => { longTitle, textContent[], displayUpdatedAt }),  unlisted  }
...
// Query: *[_type == "siteSettings" && _id == "siteSettings"][0] {    name,    shortName,    tagline,    description,    url,    copyright,    navigation[] { name, href },    heroTitle[0],    heroDescription[],    featuredPages[] -> {   "slug": slug.current,  title,  description,  "icon": icon.name,  ...select(!defined(listType) => { longTitle, textContent[], displayUpdatedAt }),  unlisted }  }
